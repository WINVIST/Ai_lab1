...
# 1. Регрессия Ridge
ridge_model = Ridge(alpha=1.0, solver='sag', max_iter=1000)
...
if len(y_train) > 1000:  # Ограничиваем размер для ARIMA
    y_train_series = pd.Series(y_train[:1000])
...
plt.xlim(0, len(y_test))  # Устанавливаем пределы оси X
